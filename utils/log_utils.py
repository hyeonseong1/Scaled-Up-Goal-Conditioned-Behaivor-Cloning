...
from ml_collections.config_flags import get_config_filename
...
def _short_env(env_name: str) -> str:
    # antmaze-large-navigate-v0 -> antmaze-large
    return "-".join(env_name.split("-")[:2])

def _short_agent(agent) -> str:
    try:
        path = get_config_filename(agent)   # e.g. "agents/gcbc.py"
        if path:
    ...
    if isinstance(agent, str):
        if agent.endswith(".py"):
            return os.path.splitext(os.path.basename(agent))[0]
        return "-".join(agent.split("-")[:2])

    return "agent"

def _get_exp_name(seed, env_name, agent):
    env_short = _short_env(env_name)
    agent_short = _short_agent(agent)
    return f"{env_short}-{agent_short}-sd{seed:03d}"
